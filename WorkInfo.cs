...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过' and recommend='是' oreder by recommendTime desc limit 6";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public int Add(Model.WorkInfo workInfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId,workName,workPicture,uploadTime,workIntroduction,workVerify,userName,activityId,recommend,recommend,recommendTime)"+"values(@workId,@workName,@workPicture,@uploadTime,@workIntrodution,@workVerify,@userName,@activityId,@recommend,@recommendTime);";
                sql += "SELECT @@IDENTITY";
        ...
        public int Update(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workName=@workName,workPicture=@workPicture,uploadTime=@uploadTime,workIntrodution=@workIntrodution,workVerify=@workVerify"+"userName=@userName,recommend=@recommend,recommendTime=@recommendTime where workId=@workId";
        ...
        public int UpdateImg(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "update workinfo set workPicture=@workPiture where workId=@workId";
                if (workinfo.workPicture == null)
                    sql = "update workinfo set workIntriduction=@workIntriduction where workId=@workId";
                return cn.Execute(sql, workinfo);
            }
        }
